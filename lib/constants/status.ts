...
export type StatusGroup = 'todo' | 'pre_production' | 'production' | 'post_production' | 'complete' | 'cancelled'
...
// Roles that can cancel or hold any project
const CANCEL_ROLES: TeamRole[] = ['strategist', 'jr_strategist', 'admin']

export const STATUS_TRANSITIONS: Record<ProjectStatus, StatusTransition[]> = {
  idea: [
    { to: 'brief', roles: ['strategist', 'jr_strategist', 'admin'] },
    { to: 'on_hold', roles: CANCEL_ROLES },
    { to: 'cancelled', roles: CANCEL_ROLES },
  ],
  on_hold: [
  ...
  posted_scheduled: [
    { to: 'ready_to_post', roles: ['admin'] }, // admin undo
  ],
  cancelled: [],
}
